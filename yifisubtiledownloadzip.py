...
# "https://yifysubtitles.ch/movie-imdb/tt0330373",
# "https://yifysubtitles.ch/movie-imdb/tt14618006",
# "https://yifysubtitles.ch/movie-imdb/tt1360190",
...
# Function to download subtitles from a single URL
def download_subtitles_streamlit(url):
    try:
        # Send an HTTP GET request to the URL
        response = requests.get(url)
                        ...
                        with requests.get(full_download_url, stream=True) as download_response:
                            if download_response.status_code == 200:
                                with open(subtitle_filepath, 'wb') as subtitle_file:
                                    for chunk in download_response.iter_content(chunk_size=1024):
                                        subtitle_file.write(chunk)
                                st.success(f"Subtitle downloaded as: {subtitle_filepath}")
                            else:
                                st.error("Failed to download the subtitle.")
                    else:
                        st.warning("Download link not found on the subtitle page.")
                else:
                    st.error(f"Failed to retrieve the subtitle page. Status code: {subtitle_response.status_code}")
            else:
                st.warning("No English subtitles found on the page.")
        else:
...
# Input for user-provided URLs
st.header("Enter Movie URLs")
url_list_input = st.text_area("Enter movie URLs (one per line)", "")
url_list = url_list_input.split('\n')

# Download button
if st.button("Download Subtitles"):
    # Use a ThreadPoolExecutor for concurrent downloads
    with ThreadPoolExecutor(max_workers=5) as executor:
        executor.map(download_subtitles_streamlit, url_list)
